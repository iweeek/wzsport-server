...
public interface StudentSportConsumeStatisticMapper {
    ...
    @Select("\n" + 
            ...
            "                        RIGHT JOIN (SELECT student_id,SUM(cost_time) AS cost_time, SUM(kcal_consumed) AS kcal_consumed,start_time\n" + 
            "                                         FROM wzsport_running_activity\n" + 
            "                                        WHERE start_time BETWEEN date(#{start}) and date(#{end})\n" + 
            "                                            AND is_valid = 1 and qualified = 1\n" + 
            ...
            "                    ) t\n" + 
            "                    GROUP BY student_id,date(sport_time)\n" + 
            "            ) AS t1  order by t1.sport_date")
    HashSet<StudentSportConsumeStatistic> getStudentSportConsumeStatisticListAll(@Param("start") Date start,@Param("end") Date end);
    
    @Select("select * from wzsport_student_sport_consume_statistic where sport_date between date(#{start}) and date(#{end})")
    HashSet<StudentSportConsumeStatistic> getAllDataList(@Param("start") Date start, @Param("end") Date end);
